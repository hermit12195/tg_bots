...
async def welcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """
    try:
        print("WELCOME STATE in WELCOME handler is triggered")
        keyboard = [
        ...
        print(f"Error in WELCOME handler - {error}")
...
async def handle_option(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    """
    try:
        print("HANDLE OPTION in WELCOME handler is triggered")
        ...
        if option == "services":
            ...
            markup = InlineKeyboardMarkup(keyboard)
            await context.bot.send_message(chat_id=update.effective_chat.id, text="❕Бажаєте, щоб про вас дізнались🤩, та залучити більше клієнтів, звертайтесь у direct 💌")
            await update.effective_message.reply_text("Вибери опцію:", reply_markup=markup)
            ...
            if client_details:
                if client_details[3] == "no":
                    ...
                    await context.bot.send_message(chat_id=query.message.chat.id, text="Вітаю🤩! Ти успішно підписався(лась) на розсилку корисних порад від мене.")
                    keyboard = [
                        [InlineKeyboardButton("Написати мені", callback_data="contact_me")],
                        [InlineKeyboardButton("Повернутись в головне меню", callback_data="back_to_menu")]
                    ]
            ...
            await context.bot.send_message(chat_id=query.from_user.id, text="Я відповідаю щодня з 10:00 до 19:00 💬: https://t.me/smar1owl")
            keyboard = [
        ...
        print(f"Error in HANDLE OPTION/WELCOME handler - {error}")
